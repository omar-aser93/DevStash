...
export default function ChaosAnimation() {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-70 md:h-70 max-md:h-55 bg-[#12121a] border border-[#1e1e2e] rounded-xl overflow-hidden"
    >
      {ICON_SVGS.map((svg, i) => (
        <div
          key={i}
          ...
          dangerouslySetInnerHTML={{ __html: `<div class="w-13.5 h-13.5">${svg}</div>` }}
        />
      ))}
    </div>
  );
