...
- Create usage limit utilities in `lib/stripe/usage.ts` with unit tests
...
### 4. Unit Tests for `lib/stripe/usage.test.ts`
...
### 7. Create `app/api/stripe/checkout/route.ts`
...
| `lib/stripe/usage.ts` | Free tier usage limit checks |
| `lib/stripe/usage.test.ts` | Unit tests for usage utilities |
| `app/api/stripe/checkout/route.ts` | Create Stripe Checkout sessions |
| `app/api/stripe/portal/route.ts` | Create Stripe Customer Portal sessions |

## Modified Files

| File | Changes |
|------|---------|
| `lib/types/next-auth.d.ts` | Add `isPro` to Session and JWT types |
| `lib/auth.ts` | Async JWT callback with `isPro` DB sync, session callback passes `isPro` |
