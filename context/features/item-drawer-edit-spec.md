...
`updateItem(itemId, data)` in `lib/actions/itemsActions.ts` following the `{ success, data, error }` return pattern. Validates input with Zod, gets session via `auth()`, validates ownership, calls query function.

## Data

- Query function in `lib/queries/items.ts` — `updateItem`
